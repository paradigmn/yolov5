    ...
    def call(self, inputs):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                y = tf.sigmoid(x[i])
                y = tf.split(y, self.na, axis=-1)
                grid = tf.transpose(self.grid[i], [0, 2, 1, 3]) - 0.5
                grid = tf.reshape(grid, [-1, ny, nx, 2])
                anchor_grid = tf.transpose(self.anchor_grid[i], [0, 2, 1, 3]) * 4
                anchor_grid = tf.split(anchor_grid, self.na, axis=2)

                tmp = []
                for j in range(self.na):
                    xy = (y[j][..., 0:2] * 2 + grid) * self.stride[i]  # xy
                    wh = y[j][..., 2:4] ** 2 * anchor_grid[j]
                    # Normalize xywh to 0-1 to reduce calibration error
                    xy /= tf.constant([[self.imgsz[1], self.imgsz[0]]], dtype=tf.float32)
                    wh /= tf.constant([[self.imgsz[1], self.imgsz[0]]], dtype=tf.float32)
                    tmp.append(tf.concat([xy, wh, y[j][..., 4:]], -1))
                y = tf.concat(tmp, axis=2)
